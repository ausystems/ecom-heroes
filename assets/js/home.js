...
if (i !== index) {
// Immediate: this click is consumed by bringing the card forward. Without it the
// video handler on this same card would also fire and autoplay a card the user
// only meant to look at. First click centres, second click plays.
...
// Both paths end in goTo(), so a swipe animates with the same 700ms transition
// the arrows use. Nothing here touches the animation itself.
const SWIPE_PX = 45;   // finger/mouse distance that counts as a swipe
const WHEEL_PX = 60;   // accumulated trackpad delta that counts as one step
let dragging = false, startX = 0, startY = 0, dx = 0, axis = null;

function endDrag() {
if (!dragging) return;
dragging = false;
if (axis === 'x' && Math.abs(dx) > SWIPE_PX) {
viewport.classList.add('was-dragged');   // swallow the click this gesture emits
goTo(index + (dx < 0 ? 1 : -1));
}
}
viewport.addEventListener('pointerdown', (e) => {
if (e.pointerType === 'mouse' && e.button !== 0) return;
if (e.target.closest && e.target.closest('.testimonial-embed')) return; // playing video
viewport.classList.remove('was-dragged');
...
if (axis === 'y') dragging = false;   // vertical intent — hand it back to the page
}
}, { passive: true });
...
// A swipe ends in a click; stop it before it reaches a card (which would play a video).
viewport.addEventListener('click', (e) => {
if (viewport.classList.contains('was-dragged')) {
e.preventDefault(); e.stopPropagation();
viewport.classList.remove('was-dragged');
}
}, true);

// Trackpad horizontal two-finger swipe. Vertical wheel is left completely alone
// so normal page scrolling (and Lenis) behave exactly as before.
let wheelAccum = 0, wheelIdle = null, wheelLock = false;
viewport.addEventListener('wheel', (e) => {
if (Math.abs(e.deltaX) <= Math.abs(e.deltaY)) return;  // vertical → not ours
e.preventDefault();
if (wheelLock) return;
wheelAccum += e.deltaX;
if (Math.abs(wheelAccum) > WHEEL_PX) {
goTo(index + (wheelAccum > 0 ? 1 : -1));
wheelAccum = 0;
wheelLock = true;                                    // one card per gesture burst
setTimeout(() => { wheelLock = false; }, 450);
}
clearTimeout(wheelIdle);
wheelIdle = setTimeout(() => { wheelAccum = 0; }, 200);
}, { passive: false });
...
gsap.set(['.eyebrow', '.cta-row', '.hero-proof'], { opacity: 0, y: 26 });
...
tl.to('.hero-proof', { opacity: 1, y: 0, duration: 0.9 }, 0.72);
if (cols.length) {
...
;
(function () {
  var cards = Array.prototype.slice.call(document.querySelectorAll('.testimonial-card--video[data-mp4]'));
  if (!cards.length) return;

  // Only one testimonial should have audio at a time.
  function pauseOthers(current) {
    cards.forEach(function (c) {
      if (c === current) return;
      var v = c.querySelector('video.testimonial-embed');
      if (v && !v.paused) v.pause();
    });
  }

  cards.forEach(function (card) {
    var src = card.getAttribute('data-mp4');
    if (!src) return;
    var video = null;

    function play(e) {
      // A click on an off-centre card is the carousel asking to bring it forward,
      // not a request to play it. Let that click through untouched.
      if (!card.classList.contains('is-active')) return;
      // Ignore the click that ends a swipe.
      if (card.closest('.testimonials-grid') &&
          card.closest('.testimonials-grid').classList.contains('was-dragged')) return;
      // Once the player exists its native controls own every click — don't intercept.
      if (video) return;
      if (e) { e.preventDefault(); e.stopPropagation(); }

      video = document.createElement('video');
      video.className = 'testimonial-embed';   // reuses the embed CSS: absolute fill, radius, z-index
      video.src = src;
      video.controls = true;
      video.playsInline = true;
      video.preload = 'auto';                  // nothing is fetched until this element is created
      video.setAttribute('aria-label', card.getAttribute('data-title') || 'Video testimonial');
      // Hold the branded thumbnail on screen while the first frame decodes, so a slow
      // connection shows the artwork rather than an empty black box.
      var thumb = card.querySelector('.tvid-thumb');
      if (thumb) video.poster = thumb.currentSrc || thumb.src;

      // Fade in on first decoded frame so there's no black flash over the poster.
      video.style.opacity = '0';
      var revealed = false;
      function reveal() {
        if (revealed) return;
        revealed = true;
        if (window.gsap) { gsap.to(video, { opacity: 1, duration: 0.5, ease: 'power2.out' }); }
        else { video.style.opacity = '1'; }
      }
      video.addEventListener('loadeddata', reveal);
      setTimeout(reveal, 2000);                // safety net: controls must never stay invisible

      video.addEventListener('error', function () {
        if (video && video.parentNode) video.parentNode.removeChild(video);
        video = null;
        card.classList.remove('is-playing');   // card returns to its poster state
      });

      // If a pending play() resolves after the viewer has already swiped away
      // (tap-play-then-swipe race), silence it the moment playback starts.
      video.addEventListener('play', function () {
        if (!card.classList.contains('is-active')) video.pause();
      });

      (card.querySelector('.tcard-media') || card).appendChild(video);
      card.classList.add('is-playing');
      pauseOthers(card);
      var p = video.play();
      if (p && p.catch) p.catch(function () {});  // if autoplay is vetoed, native controls remain
    }
    ...
    // Swiping to another card silences this one; its state is untouched otherwise,
    // so coming back resumes exactly where the viewer left off.
    new MutationObserver(function () {
      if (!card.classList.contains('is-active') && video && !video.paused) video.pause();
...
;
(function () {
var zone = document.querySelector('.hero-right');
if (!zone || !(window.matchMedia && matchMedia('(pointer: coarse)').matches)) return;
var t = null;
// A finger on the hero cards pauses their auto-scroll so the content can be read;
// it resumes shortly after the touch ends.
zone.addEventListener('touchstart', function () {
clearTimeout(t); zone.classList.add('is-held');
}, { passive: true });
function release() { clearTimeout(t); t = setTimeout(function () { zone.classList.remove('is-held'); }, 1400); }
...
;
(function () {
var mq = window.matchMedia && matchMedia('(max-width: 760px)');
if (!mq || !mq.matches) return;
var reduce = matchMedia('(prefers-reduced-motion: reduce)').matches ||
document.documentElement.classList.contains('low-power');
if (reduce) return;                       // static first card is the calm fallback
var track = document.querySelector('.hero-right .col-1 .col-track');
var zone  = document.querySelector('.hero-right');
if (!track || !zone) return;
var members = track.children;
if (members.length < 2) return;
var half = Math.floor(members.length / 2); // track content is doubled for the old loop
var idx = 0;
var base = 0;                              // natural gap between card 0 and the window top

function currentShift() {
var t = track.style.transform, m = t && t.match(/-?[\d.]+/);
return m ? parseFloat(m[0]) : 0;
}
function align(animate) {
var target = base + (members[idx].offsetTop - members[0].offsetTop);
track.style.transition = animate ? 'transform 700ms cubic-bezier(.22,.61,.36,1)' : 'none';
track.style.transform = 'translateY(' + (-target) + 'px)';
}
function measureBase() {
// distance from the window top to card 0, via the offset chain:
// layout-based, so it ignores the transform and stays correct even
// if it runs mid-transition
var y = 0, n = members[0];
while (n && n !== zone) { y += n.offsetTop; n = n.offsetParent; }
base = Math.round(y);
}
function remeasure() { measureBase(); align(false); }
measureBase();
align(false);                              // card 0 fills the window from the first paint
window.addEventListener('resize', remeasure, { passive: true });
// late layout shifts (web fonts swapping in, images decoding) change the
// card offsets after init — re-align when they settle so every step
// lands flush with the window
...
function step() {
if (zone.classList.contains('is-held')) return;   // finger on the cards: hold position
idx++;
align(true);
if (idx === half) {                                // seam: clone of card 0 — snap back
setTimeout(function () { idx = 0; align(false); }, 750);
}
}
