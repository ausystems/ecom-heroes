...
// distance from the window top to card 0 with the transform removed
base = Math.round(members[0].getBoundingClientRect().top - currentShift() - zone.getBoundingClientRect().top);
}
measureBase();
align(false);                              // card 0 fills the window from the first paint
window.addEventListener('resize', function () { measureBase(); align(false); }, { passive: true });
