...
/* Mobile brand-logo flow: merge the two marquee columns into one track
   so the logos stream upward through a masked window, exactly like the
   hero cards flow on mobile. Skipped for reduced-motion / low-power —
   the CSS static column is the calm fallback there. */
(function () {
'use strict';
...
var desktopHTML = logos.innerHTML;
var built = false;
function build() {
if (built) return;
var cols = logos.querySelectorAll('.hlogo-col');
if (cols.length < 2) return;
var first = [], second = [];
[first, second].forEach(function (bucket, ci) {
var tiles = cols[ci].querySelectorAll('.hlogo-tile');
var half = Math.floor(tiles.length / 2) || tiles.length;
for (var i = 0; i < half; i++) bucket.push(tiles[i].cloneNode(true));
});
// alternate the columns' brands so the single stream keeps the mix
var mixed = [];
...
col.className = 'hlogo-col up';
var track = document.createElement('div');
track.className = 'hlogo-track';
mixed.forEach(function (t) { track.appendChild(t); });
mixed.forEach(function (t) { track.appendChild(t.cloneNode(true)); }); // clones make the loop seamless
col.appendChild(track);
logos.innerHTML = '';
logos.appendChild(col);
logos.classList.add('is-flow');
built = true;
}
function restore() {
if (!built) return;
logos.innerHTML = desktopHTML;
logos.classList.remove('is-flow');
built = false;
}
function apply() { if (mq.matches) build(); else restore(); }
apply();
if (mq.addEventListener) mq.addEventListener('change', apply);
else mq.addListener(apply);
// a finger on the logos pauses the flow so a tile can be tapped,
// mirroring the hero-card behaviour on the homepage
var t = null;
logos.addEventListener('touchstart', function () {
clearTimeout(t); logos.classList.add('is-held');
}, { passive: true });
function release() {
clearTimeout(t);
t = setTimeout(function () { logos.classList.remove('is-held'); }, 1400);
}
logos.addEventListener('touchend', release, { passive: true });
logos.addEventListener('touchcancel', release, { passive: true });
})();
